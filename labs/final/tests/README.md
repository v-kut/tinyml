...
| `test_ml_pipeline.py`        | `ml/regression/`: the episode-level train/val split and `ObsNorm` leakage, `Frame` straddle, `reward_clip` units, hand-computed returns-to-go                                                                                                                                 |
| `test_snapshot_roundtrip.py` | `ml/snapshot.py`: normalization equals `VecNormalize`'s, a loaded snapshot's `act` equals `model.predict`, the version gate, publish atomicity                                                                                                                                |
