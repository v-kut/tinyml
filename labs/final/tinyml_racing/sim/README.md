...
| `track.py`       | `TrackConfig`, `Track`, `generate_track`: a convex hull pulled into corners, stretched to plant one main straight, filleted into arcs and straights, sampled at even arc length. Also `min_steerable_corner_radius` and `default_corner_speed_radius`, both read off the car                 |
